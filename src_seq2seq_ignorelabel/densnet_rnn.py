...
def DenseNetFCNLSTM(input_shape, nb_dense_block=5, growth_rate=16, nb_layers_per_block=4,
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1E-4):
    ...
    if bottleneck:
        ...
        x = ConvLSTM2D(inter_channel, (1, 1), kernel_initializer='he_uniform', padding='same', use_bias=False,
                          kernel_regularizer=l2(weight_decay),return_sequences=True)(x)
    ...
    x = ConvLSTM2D(nb_filter, (3, 3), kernel_initializer="he_uniform", padding="same", use_bias=False,
                      kernel_regularizer=l2(weight_decay),return_sequences=True)(x)
    if dropout_rate:
        x = Dropout(dropout_rate)(x)
...
def __transition_block(ip, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4):
    ...
    x = BatchNormalization(axis=concat_axis, gamma_regularizer=l2(weight_decay),
                           beta_regularizer=l2(weight_decay))(ip)
    x = Activation('relu')(x)
    x = ConvLSTM2D(int(nb_filter * compression), (1, 1), kernel_initializer="he_uniform", padding="same", use_bias=False,
                      kernel_regularizer=l2(weight_decay),return_sequences=True)(x)
    if dropout_rate:
        x = Dropout(dropout_rate)(x)
...
def __transition_up_block(ip, nb_filters, type='upsampling', output_shape=None, padding_param = 'same', weight_decay=1E-4):
    ...
    if type == 'upsampling':
        ...
        x = ConvLSTM2D(nb_filters, (3, 3), activation="relu", padding='same', kernel_regularizer=l2(weight_decay),
                          use_bias=False, kernel_initializer='he_uniform',return_sequences=True)(ip)
        x = SubPixelUpscaling(scale_factor=2)(x)
        x = ConvLSTM2D(nb_filters, (3, 3), activation="relu", padding='same', kernel_regularizer=l2(weight_decay),
                          use_bias=False, kernel_initializer='he_uniform',return_sequences=True)(x)
    elif type == 'atrous':
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    x = ConvLSTM2D(init_conv_filters, (3, 3), kernel_initializer="he_uniform", padding="same", name="initial_conv2D", use_bias=False,
                      kernel_regularizer=l2(weight_decay),return_sequences=True)(img_input)
    ...
    if include_top:
        x = ConvLSTM2D(nb_classes, (1, 1), activation='softmax', padding='same', kernel_regularizer=l2(weight_decay),
                          use_bias=False)(x)
    return x

if __name__ == '__main__':
    model = DenseNetFCNLSTM((32, 32, 28), nb_dense_block=2, growth_rate=16, dropout_rate=0.2,
